...
/// Incremental SHA-256. Used directly where the message is built from several
/// pieces (HMAC, the signing input of a token) to avoid concatenating buffers.
#[derive(Clone)]
pub struct Sha256 {
    state: [u32; 8],
    buffer: [u8; 64],
...
impl Sha256 {
    ...
    /// Append bytes without counting them toward the message length. Only the
    /// padding uses this.
